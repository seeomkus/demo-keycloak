# Project Overview — Keycloak SSO Demo

This document is a **complete, self-contained overview** of this project — what was built, why, how it's structured, and what state it's currently in. It does not assume you've read any other document, and it does not link out anywhere else — everything you need for the big picture is right here.
...
7. [Key Findings & Fixes](#7-key-findings--fixes)
8. [Current Status](#8-current-status)
9. [Limitations & Security Notes](#9-limitations--security-notes)
...
**What sets this project apart from a typical tutorial:** every claim is backed by **real technical evidence** (actual HTTP responses, actual server logs), and every bug found along the way is **documented as-is** — including design mistakes that were actually made and how they were fixed. This isn't a staged, frictionless scenario — it's a real build process with all its real obstacles, and all documentation (including this document) is written in English.
...
| Frontend — Portal | Next.js 16 (App Router) + Auth.js v5 | 3088 | Main demo app, hosts the interactive live-demo page |
...
Every component runs as a native Windows process — managed via `.bat` scripts, no PowerShell involved in day-to-day operation.
...
A single interactive page on the Portal application (`/flow-demo`) that demonstrates the **entire SSO flow for real**, not an animation or simulation:
...
| Script | Function |
|---|---|
| `start.bat` | Starts every service, in the background, with no new terminal windows |
| `stop.bat` | Stops services by the port they're using (precise, doesn't kill unrelated processes) |
| `status.bat` | Live status check of every service |
| `restart.bat` | `stop` then `start` |
| `update-ip.bat` | Auto-detects the current LAN IP and syncs it into every application/Keycloak client config |
...
Keycloak itself is **deliberately installed outside the project folder**, since it's a standalone server, not an application dependency.
...
| Item | Status |
|---|---|
| Full build (Keycloak + Portal + Admin + FastAPI + PostgreSQL) | ✅ Complete |
